...
def mostra_qtnd_linhas(dados):
   ...
   st.write(dados.head(qntd_linhas).style.format(subset = ['Valor'], formatter = '{:.2f}'))

#criando a função plot_estoque
def plot_estoque(dados, categoria):

    dados_plot = dados.query('Categoria == @categoria')
...
if categoria != 'Todas':
    ...
    mostra_qtnd_linhas(df_categoria)
else:
    mostra_qtnd_linhas(dados)

#filtro para o gráfico

st.sidebar.markdown('## Filtro para o gráfico')

categoria_grafico = st.sidebar.selectbox('Selecione a categoria para aprensentar no gráfico', options = dados['Categoria'].unique())
